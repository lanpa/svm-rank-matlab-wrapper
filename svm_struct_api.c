...
SAMPLE      read_struct_examples_matlab(char *file, STRUCT_LEARN_PARM *sparm, double* label_in, double* qid_in, float* feature, int nSample, int nFeature)
{
    ...
    /*
    for (int i = 0;i<10;i++){
        printf("data: %d target:%f qid:%f",i,label_in[i], qid_in[i]);
    ...
    */
    ...
    mexPrintf("inside read_struct_examples\n");
    //read_documents_from_matlab(file,&instances,&labels,&totwords,&n);
    //read_documents_from_matlab
    instances = (DOC **)my_malloc(sizeof(DOC *)*(nSample+2));    /* feature vectors */
    WORD* words = (WORD *)my_malloc(sizeof(WORD)*(nFeature+10));
    n = nSample;
    totwords = nFeature;
    for (int dnum = 0;dnum<nSample;dnum++){
        //printf("%d\n:", dnum);
        for(int j = 0;j<nFeature;j++){
            words[j].wnum = j+1;
            words[j].weight = feature[dnum*nFeature+j];
	    //printf("%f\n", words[j].weight);
        }
        words[nFeature].wnum = 0; //(words[wpos]).wnum=0; don't know why
        //
        instances[dnum] = create_example(dnum,(long)qid_in[dnum],0,1,
                          create_svector(words,"",1.0));
        //printf("done\n");
    }
    printf("all samples read into mex memory (malloc)\n");
    labels = label_in;
    ...
    for(i=0;i<n;i++) {
        ...
        if(instances[i]->queryid != qid) { //start of different pairs. qid keeps previous value!  
            if(instances[i]->queryid < qid) {
                printf("ERROR: Query ID's in data file have to be in increasing order (line %ld)!\n",i);
                exit(1);
            }
            //printf("HIHIHIHI\n");
            //create new x, y and initialize everything
            qid=instances[i]->queryid;
            sample.n++;
            examples=(EXAMPLE *)realloc(examples,sizeof(EXAMPLE)*sample.n);
            x=&examples[sample.n-1].x;//pointer to pattern
            y=&examples[sample.n-1].y;//pointer to label
            x->doc=NULL;
            x->totdoc=0;
            y->factor=NULL;
            y->class=NULL;
